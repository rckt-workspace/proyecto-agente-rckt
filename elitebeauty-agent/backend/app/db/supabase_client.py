...
# El cliente supabase-py usado aquí es síncrono (bloqueante) por dentro, aunque
# estas funciones estén declaradas `async def`. Cada consulta corre en un hilo
# aparte (asyncio.to_thread) para no congelar el event loop mientras espera la
# respuesta de red — así el backend puede atender varias llamadas/mensajes en
# paralelo de verdad, en vez de encolarlos detrás de cada consulta a la DB.
